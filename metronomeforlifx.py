...
global spotify_token, client_id, client_secret, lifx_token, colors
colors = []
client_id = ""
client_secret = ""
lifx_token = ""
...
def main():
    ...
    global client_secret, colors, client_id, lifx_token
...
def play_song():
    rand1 = random.randint(0, (len(colors) - 2))
    firstColor = colors[rand1]
    del colors[rand1]
...
def spotify_authenticate():
    ...
    token = util.prompt_for_user_token("codycoogan", scope, client_id, client_secret, "https://www.google.com/")
    if token:
        spotify_token = token
    else:
